...
from env_cartesian.load_data import get_index
...
class HGNNScheduler(nn.Module):
    ...
    def get_normalized(self, raw_opes, raw_mas, proc_time, batch_idxes, nums_opes, flag_sample=False, flag_train=False):
        ...
        if not flag_sample and not flag_train:
            ...
            for i in range(batch_size):
                # print("raw_opes[i]=",raw_opes[i])
                # print("raw_opes[i, :nums_opes[i], :]=",raw_opes[i, :nums_opes[i], :])
            ...
            print("mean_opes=",mean_opes)
            # print("mean_opes.size()=", mean_opes.size())
        ...
        # DRL-S and scheduling during training have a consistent number of operations
            ...
            print("mean_opes=", mean_opes)
            print("mean_opes.size()=", mean_opes.size())
            print("mean_mas.size()=", mean_opes.size())
            print("std_opes.size()=", std_opes.size())
            print("std_mas.size()=", std_mas.size())
    ...
    def get_action_prob(self, state, memories, flag_sample=False, flag_train=False):
        ...
        # Raw feats
        print("state.feat_opes_batch[batch_idxes].size()=",state.feat_opes_batch[batch_idxes].size())
        print("state.feat_mas_batch[batch_idxes].size()=", state.feat_mas_batch[batch_idxes].size())
        raw_opes = state.feat_opes_batch.transpose(1, 2)[batch_idxes]
        raw_mas = state.feat_mas_batch.transpose(1, 2)[batch_idxes]
        print("state.feat_opes_batch[batch_idxes].size()=", state.feat_opes_batch[batch_idxes].size())
        print("state.feat_mas_batch[batch_idxes].size()=", state.feat_mas_batch[batch_idxes].size())
        print("raw_opes.size()=",raw_opes.size())
        print("raw_mas.size()=", raw_mas.size())
        ...
        # shape: [len(batch_idxes), num_jobs, num_mas]
        ma_eligible = ~state.mask_ma_procing_batch[batch_idxes].unsqueeze(1).expand_as(h_opes_padding[..., 0])
        # print("ma_eligible.size()=",ma_eligible.size())
        ...
        ope_eligible = torch.full(size=(batch_size, state.num_opes), dtype=torch.bool, fill_value=False)
        for i in range(batch_size):
            for ii in range(state.num_opes):
                ii_in_later_index = get_index(state.later_batch[i], ii)
                if len(ii_in_later_index) == 0:
                    if not (state.mask_ope_finish_batch[i][ii] or state.mask_ope_procing_batch[i][ii]):
                        ope_eligible[i][ii] = True
                else:
                    all_former_done = []
                    for former_ii in range(len(ii_in_later_index)):
                        all_former_done.append(state.mask_ope_finish_batch[i][int(state.former_batch[i]
                                                                                  [ii_in_later_index[former_ii]])])
                    all_former_done = torch.tensor(all_former_done)
                    if all_former_done.all():
                        ope_eligible[i][ii] = True
                    if state.mask_ope_finish_batch[i][ii] or state.mask_ope_procing_batch[i][ii]:
        ...
        # print("ope_eligible.size()=", ope_eligible.size())
        ope_eligible = ope_eligible.expand_as(h_opes_padding[..., -1])  ##
        # print("ope_eligible.size()=",ope_eligible.size())

        # shape: [len(batch_idxes), num_jobs, num_mas]
        eligible = ope_eligible & ma_eligible & (eligible_proc == 1)
        # print("eligible.size()=",eligible.size())
        if (~(eligible)).all():
    ...
    def act(self, state, memories, dones, flag_sample=True, flag_train=True):
        ...
        batch_size=state.batch_idxes.size(0)
        # print("batch_size=",batch_size)
        ...
        # print("nums_ope_batch=",state.nums_ope_batch)
        cum_ope_batch=[]
        for i in range(batch_size):
            cum_ope_batch.append(torch.cumsum(state.nums_ope_batch[i],0))
        cum_ope_batch=torch.stack(cum_ope_batch,0)
        # print("cum_ope_batch=",cum_ope_batch)
        ...
        # print("action_indexes",action_indexes)
        mas = (action_indexes / state.mask_ope_finish_batch.size(1) + 1e-3).long()
        opes = (action_indexes % state.mask_ope_finish_batch.size(1)).long()
        # print( (action_indexes / state.mask_ope_finish_batch.size(1)))
        # print("state.mask_ope_finish_batch.size(1)=",state.mask_ope_finish_batch.size(1))
        # print("mas=", mas)
        # print("opes=",opes)
        jobs=np.zeros(batch_size)
        for i in range(batch_size):
            for j in range(len(cum_ope_batch[i])):
                if opes[i]< cum_ope_batch[i][j]:
                    jobs[i]=j
                    break
        jobs=torch.tensor(jobs).int()
        # print("jobs=",jobs)
    ...
    def evaluate(self, ope_ma_adj, ope_pre_adj, ope_sub_adj, raw_opes, raw_mas, proc_time,
        ...
        h_opes_padding = h_opes.unsqueeze(-2).expand(-1, -1, proc_time.size(-1), -1)
        # print("h_opes_padding=", h_opes_padding)  ## 26 x 5 x 8
