...
from env_dag_train.load_data import nums_detec
from env_dag_train.load_data import nums_detec_dag
...
def main():
    ...
    with open("./config_dag.json", 'r') as load_f:
    ...
    memories = PPO_model_dag_train.Memory()
    model = PPO_model_dag_train.PPO(model_paras, train_paras)
    ...
    save_path = './save_dag_new/test_{0}'.format(str_time)
    ...
    for i_rules in range(len(rules)):
        ...
        for i_ins in range(num_ins):
            ...
            # Create environment object
                ...
                if test_paras["sample"]:
                    env = gym.make('fjsp-v2', case=[test_file] * test_paras["num_sample"],
                                   env_paras=env_test_paras, data_source='file')
                # DRL-G, each env contains one instance
                else:
                    print("test_file=",[test_file])
                    print("env_test_paras=",env_test_paras)
                    env = gym.make('fjsp-v2', case=[test_file], env_paras=env_test_paras, data_source='file')
