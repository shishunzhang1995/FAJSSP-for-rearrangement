...
from env_dag.load_data import load_fjs, nums_detec,get_index,start_time,start_time_estimate
import numpy as np
...
class EnvState:
    ...
    def update(self, batch_idxes, feat_opes_batch, feat_mas_batch, proc_times_batch, ope_ma_adj_batch,
               mask_ope_procing_batch, mask_ope_finish_batch, mask_ma_procing_batch, ope_step_batch, time):
...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        # print("len(tensors[7])=",len(tensors[7][0]))
        # print("len(tensors[0])=", len(tensors[0][0]))
        print("former=",self.former)
        print("later=",self.later)
        print("self.nums_ope=",self.nums_ope)
        ...
        self.success_time_ope_batch =torch.zeros(size=(self.batch_size,self.num_opes))  ##26 0
        ...
        self.mask_ope_procing_batch = torch.full(size=(self.batch_size, num_opes), dtype=torch.bool, fill_value=False)
        # shape: (batch_size, num_jobs), True for completed jobs
        self.mask_ope_finish_batch = torch.full(size=(self.batch_size, num_opes), dtype=torch.bool, fill_value=False)
        ...
        self.state = EnvState(batch_idxes=self.batch_idxes,
                              ...
                              num_opes=self.num_opes,former_batch=torch.Tensor(self.former),
    ...
    def step(self, actions):
        ...
        # print("last_opes=",last_opes)
        # print("self.num_ope_biases_batch[self.batch_idxes, jobs]=",self.num_ope_biases_batch[self.batch_idxes, jobs])

        # print("self.cal_cumul_adj_batch[self.batch_idxes]",
        ...
        for i in range(self.batch_idxes.size(0)):
            ...
            # print("self.feat_opes_batch[self.batch_idxes[i], 3, start_ope[i]:end_ope[i]+1]=",
            #       self.feat_opes_batch[self.batch_idxes[i], 3, start_ope[i]:end_ope[i] + 1]
            #       )
        ...
        print("self.time=",self.time)
        print("estimate_end_time_opes_batch=",estimate_end_time_opes_batch)
        print("self.end_ope_biases_batch[self.batch_idxes, :]=",self.end_ope_biases_batch[self.batch_idxes, :])
        # for i in range(self.batch_size):
        #     for ii in range(self.num_opes):
        #         if is_scheduled[i][ii] and self.time[i]>=estimate_end_time_opes_batch[i][ii] and self.success_time_ope_batch[i][ii]<1:
        #             self.mask_ope_procing_batch[i][ii] = False
        #             self.mask_ope_finish_batch[i][ii]= True
        #             self.mask_ma_procing_batch=torch.where(self.ope_ma_adj_batch[i][ii]==1,False,
        #                                                    self.mask_ma_procing_batch)
        #             self.success_time_ope_batch[i][ii]+=1
        ...
        print("(flag_trans_2_next_time==0) & (~self.done_batch)=",(flag_trans_2_next_time==0) & (~self.done_batch))
        print("~((flag_trans_2_next_time==0) & (~self.done_batch))=",~((flag_trans_2_next_time==0) & (~self.done_batch)))
        print("(~((flag_trans_2_next_time==0) & (~self.done_batch))).all()=",
              (~((flag_trans_2_next_time==0) & (~self.done_batch))).all())
        print("~((~((flag_trans_2_next_time==0) & (~self.done_batch))).all())=",
              ~((~((flag_trans_2_next_time==0) & (~self.done_batch))).all()))
        ...
        while ~((~((flag_trans_2_next_time==0) & (~self.done_batch))).all()):
            ...
            # for i in range(self.batch_size):
            #     for ii in range(self.num_opes):
            #         if is_scheduled[i][ii] and self.time[i] >= estimate_end_time_opes_batch[i][ii] and \
            #                 self.success_time_ope_batch[i][ii] < 1:
            #             self.mask_ope_procing_batch[i][ii] = False
            #             self.mask_ope_finish_batch[i][ii] = True
            #             self.mask_ma_procing_batch = torch.where(self.ope_ma_adj_batch[i][ii] == 1, False,
    ...
    def if_no_eligible(self):
        ...
        # print("self.ope_step_batch",self.ope_step_batch)
        # print("self.end_ope_biases_batch",self.end_ope_biases_batch)
        ...
        ##  self.ope_step_batch tensor([[ 1,  9, 14, 19]])  the operation can be scheduled next
        # op_proc_time = self.proc_times_batch.gather(1, ope_step_batch.unsqueeze(-1).expand(-1, -1,
        #                                                                                 self.proc_times_batch.size(2)))
        ##  op_proc_time= tensor([[[77.,  0., 70., 66.,  0.],  the proc time  of the operation can be scheduled next
        # print("op_proc_time",op_proc_time)
        # print("self.mask_ma_procing_batch=",self.mask_ma_procing_batch)
        # print("self.mask_ope_procing_batch=", self.mask_ope_procing_batch)  ## ope 0 is procing
        # print("self.mask_ope_finish_batch=", self.mask_ope_finish_batch)
