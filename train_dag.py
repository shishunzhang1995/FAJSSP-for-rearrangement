...
from env.case_generator import CaseGenerator
from validate_dag import validate, get_validate_env
...
def main():
    ...
    with open("./config_dag_train.json", 'r') as load_f:
    ...
    model = PPO_model_dag_train.PPO(model_paras, train_paras, num_envs=env_paras["batch_size"]) ##20
    ...
    save_path = './save_dag_train/train_{0}'.format(str_time)
    ...
    for i in range(1, train_paras["max_iterations"]+1):  ## 1-1000
        ...
        if (i - 1) % train_paras["parallel_iter"] == 0:
            ...
            print("sample=",sample)
            env = gym.make('fjsp-v2', case=sample, env_paras=env_paras,data_source='file') ##batch 20
